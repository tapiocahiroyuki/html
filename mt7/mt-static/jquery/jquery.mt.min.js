...
return false;}}});return!validator.error;},raise:function(msg){this.error=true;this.errstr=msg||trans('Invalid value');return false;},validClass:'valid',errorClass:'is-invalid',doFocus:true,wrapError:function($target,msg){return $('<label/>')
...
if(iframeCountBeforeRemove<=1){return true;}else{e.preventDefault();e.stopImmediatePropagation();$iframe.remove();return false;}});$modal.on('hidden.bs.modal',function(e){var $iframe=window.top.jQuery('.mt-modal iframe:last-child');$iframe.remove();});window.top.jQuery(window.top).on('resize',resizeModal);}
...
return id;}
function getIframe(){var id=getNextIframeId();var html='<iframe class="mt-dialog-iframe embed-responsive-item" name="modal-'+id+'"></iframe>';var $iframe=window.top.jQuery(html);$iframe.on('load',resizeModal);return $iframe;}
...
function resizeModal(){var modalHeight;var $iframeContents=window.top.jQuery('iframe.embed-responsive-item:visible').contents();if($iframeContents.find('body .modal-body').length>0){modalHeight=$iframeContents.find('body').outerHeight(true);}else{modalHeight=$iframeContents.find('body > *:first').outerHeight(true);}
if(modalHeight<500){modalHeight=500;}
$('.mt-modal .modal-content').css('padding-bottom',modalHeight);}})(jQuery);
